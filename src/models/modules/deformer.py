...
class MultiHeadAttention(nn.Module):
    def __init__(self, d_model=256, n_head=4, d_qkv=32, dropout=0.1, **kwargs):
        ...
        self.w_qk = nn.ParameterList([
            nn.Parameter(torch.Tensor(d_model, d_qkv)),
            nn.Parameter(torch.Tensor(d_model, d_qkv)),
            nn.Parameter(torch.Tensor(n_head, d_qkv))
        ])
        self.w_vo = nn.ParameterList([
            nn.Parameter(torch.Tensor(d_model, d_qkv)),
            nn.Parameter(torch.Tensor(d_model, d_qkv)),
            nn.Parameter(torch.Tensor(n_head, d_qkv))
        ])

        for item in itertools.chain(self.w_qk, self.w_vo):
            nn.init.xavier_normal_(item)
    ...
    def forward(self, x, mask):
        ...
        scores = oe.contract('bim,mr,nr,hr,bjn->bhij', *[x, *self.w_qk, x], optimize='optimal') / math.sqrt(self.d_qkv) # (batch size, n_head, length, length)
        if mask is not None:
