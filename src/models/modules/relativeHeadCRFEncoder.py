...
class FullyDecomposedHeadProbEncoder(nn.Module):
    """
    Head Probabilistic Transformer encoder.

    Fully decomposed on distance dimension.
    """
    def __init__(self, 
            d_model: int = 32, 
            n_head: int = 10, 
        ...
        :param output_prob: If true, output a normalized probabilistic distribution. Otherwise
                            output unnormalized scores.
        :param use_td: control tensor decomposition. Always decompose on distance. Options:
                         - 'no:{rank}': no tensor decomposition;
                         - 'uv:{rank}': each 'head' decompose to 2 matrices W = U @ V. Use a
                           number to set the rank, e.g. 'uv:64';
                         - 'uvw:{rank}': decompose to sum of product of 3 vectors 
                           W = \sum U * V * W, where * is the outer product. Use a number to set
        ...
        if self.use_td.startswith('uv:'):
            rank = int(self.use_td.split(':')[-1])
            self.D = nn.Parameter(torch.Tensor(2*self.dists, rank))
            self.U = nn.Parameter(torch.Tensor(self.d_model, self.n_head, rank))
            self.V = nn.Parameter(torch.Tensor(self.d_model, self.n_head, rank))
            ...
            rank = int(self.use_td.split(':')[-1])
            self.D = nn.Parameter(torch.Tensor(2*self.dists, rank))
            self.UVW = nn.Parameter(torch.Tensor(self.d_model, self.d_model, self.n_head, rank))
            nn.init.normal_(self.D)
            nn.init.normal_(self.UVW)
    ...
    def forward(self, x, mask):
        ...
        rank = int(self.use_td.split(':')[-1])
        params = torch.cat((torch.zeros(1, rank, device=x.device), self.D), dim=0)
        distmask = params[distance]

        ## Unary score
        unary: torch.Tensor = x # (batch_size, max_len, d_model)

        ## Recover ternary score
        if self.use_td.startswith('uv:'):
            expr_F = oe.contract_expression('zia,zjb,acd,bcd,ijd->zcij',*[unary.shape, unary.shape, self.U, self.V, distmask], constants=[2,3,4], optimize='optimal')
            expr_G1 = oe.contract_expression('zjb,zcij,acd,bcd,ijd->zia', *[unary.shape, (batch_size, self.n_head, max_len, max_len), self.U, self.V, distmask], constants=[2,3,4], optimize='optimal')
            expr_G2 = oe.contract_expression('zjb,zcji,bcd,acd,jid->zia', *[unary.shape, (batch_size, self.n_head, max_len, max_len), self.U, self.V, distmask], constants=[2,3,4], optimize='optimal')
        elif self.use_td.startswith('uvw:'):
            expr_F = oe.contract_expression('zia,zjb,ad,bd,cd,ijd->zcij',*[unary.shape, unary.shape, self.U, self.V, self.W, distmask], constants=[2,3,4,5], optimize='optimal')
            expr_G1 = oe.contract_expression('zjb,zcij,ad,bd,cd,ijd->zia', *[unary.shape, (batch_size, self.n_head, max_len, max_len), self.U, self.V, self.W, distmask], constants=[2,3,4,5], optimize='optimal')
            expr_G2 = oe.contract_expression('zjb,zcji,bd,ad,cd,jid->zia', *[unary.shape, (batch_size, self.n_head, max_len, max_len), self.U, self.V, self.W, distmask], constants=[2,3,4,5], optimize='optimal')
        else:
            expr_F = oe.contract_expression('zia,zjb,abcd,ijd->zcij',*[unary.shape, unary.shape, self.UVW, distmask], constants=[2,3], optimize='optimal')
            expr_G1 = oe.contract_expression('zjb,zcij,abcd,ijd->zia', *[unary.shape, (batch_size, self.n_head, max_len, max_len), self.UVW, distmask], constants=[2,3], optimize='optimal')
            expr_G2 = oe.contract_expression('zjb,zcji,bacd,jid->zia', *[unary.shape, (batch_size, self.n_head, max_len, max_len), self.UVW, distmask], constants=[2,3], optimize='optimal')
    ...
    def getTernaryNorm(self, p):
        ## Recover ternary score
        if self.use_td.startswith('uv:'):
            ternary = oe.contract('acd,bcd->abcd', *[self.U, self.V], backend='torch')
        elif self.use_td.startswith('uvw:'):
            ternary = oe.contract('ad,bd,cd->abcd', *[self.U, self.V, self.W], backend='torch')
        else:
            ternary = self.UVW
