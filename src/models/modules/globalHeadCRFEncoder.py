    ...
    def __init__(self, 
        ...
        :param use_td_global: simulate the global node. This is the same as doing a tensor decomposition.
                         ...
                         - 'norm:{rank}': similar to the above but matrix U is normalized.
                           e.g. 'norm:64';
                       Default: 'no'.
        :param mode: mode for global nodes. Opitons: 'all-attn', 'attn-split', 'single-channel'. Default: 'all-attn'.
        """
        super().__init__()
        ...
        assert self.mode in ('all-attn', 'attn-split', 'single-channel'), f"Unexpected mode: {self.mode}"
