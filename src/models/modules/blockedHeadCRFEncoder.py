...
class BlockHeadProbEncoder(nn.Module):
    """
    Block Head Probabilistic Transformer encoder.
    Iterations will not share the parameters.
    """
    def __init__(self, 
            d_model: int = 32, 
            n_head: int = 10, 
        ...
        if self.use_td.startswith('uv:'):
            ...
            self.U = nn.Parameter(torch.Tensor(self.n_iter, 2*(len(self._dists)+1), self.d_model, rank, self.n_head))
            self.V = nn.Parameter(torch.Tensor(self.n_iter, 2*(len(self._dists)+1), self.d_model, rank, self.n_head))
            nn.init.normal_(self.U, std = 1/math.sqrt(rank))
            nn.init.normal_(self.V, std = 1/math.sqrt(rank))
        elif self.use_td.startswith('uvw:'):
            rank = int(self.use_td.split(':')[-1])
            self.U = nn.Parameter(torch.Tensor(self.n_iter, 2*(len(self._dists)+1), self.d_model, rank))
            self.V = nn.Parameter(torch.Tensor(self.n_iter, 2*(len(self._dists)+1), self.d_model, rank))
            self.W = nn.Parameter(torch.Tensor(self.n_iter, 2*(len(self._dists)+1), self.n_head, rank))
            nn.init.normal_(self.U, std = 1/math.sqrt(rank))
            nn.init.normal_(self.V, std = 1/math.sqrt(rank))
            nn.init.normal_(self.W, std = 1/math.sqrt(rank))
        else:
            self.ternary = nn.Parameter(torch.Tensor(self.n_iter, 2*(len(self._dists)+1), self.d_model, self.d_model, self.n_head))
    ...
    def forward(self, x, mask):
        ...
        if self.use_td.startswith('uv:'):
            ternary = oe.contract('mkadc,mkbdc->mkabc', *[self.U, self.V], backend='torch')
        elif self.use_td.startswith('uvw:'):
            ternary = oe.contract('mkad,mkbd,mkcd->mkabc', *[self.U, self.V, self.W], backend='torch')
        else:
        ...
        for iteration in range(self.n_iter):
            ...
            if self.async_update:
                ...
                second_order_message_F = oe.contract('zia,zjb,kabc,kij->zcij',*[q_z, q_z, ternary[iteration], distmask], backend='torch')
                ...
                # Calculate 2nd message for different dists
                second_order_message_G = oe.contract('zjb,zcij,kabc,kij->zia', *[q_z, q_h, ternary[iteration], distmask], backend='torch')
                if not self.block_msg:
                    second_order_message_G = second_order_message_G + oe.contract('zjb,zcji,kbac,kji->zia', *[q_z, q_h, ternary[iteration], distmask], backend='torch')
...
class DoubleBlockHeadProbEncoder(BlockHeadProbEncoder):
    """
    Block Head Probabilistic Transformer encoder.
    Iterations will not share the parameters. In iterations, message passing also
    does not share parameters.
    """
    def __init__(self, 
            d_model: int = 32, 
            n_head: int = 10, 
        ...
        if self.use_td.startswith('uv:'):
            rank = int(self.use_td.split(':')[-1])
            self.U = nn.Parameter(torch.Tensor(2*self.n_iter, 2*(len(self._dists)+1), self.d_model, rank, self.n_head))
            self.V = nn.Parameter(torch.Tensor(2*self.n_iter, 2*(len(self._dists)+1), self.d_model, rank, self.n_head))
            nn.init.normal_(self.U, std = 1/math.sqrt(rank))
            nn.init.normal_(self.V, std = 1/math.sqrt(rank))
        elif self.use_td.startswith('uvw:'):
            rank = int(self.use_td.split(':')[-1])
            self.U = nn.Parameter(torch.Tensor(2*self.n_iter, 2*(len(self._dists)+1), self.d_model, rank))
            self.V = nn.Parameter(torch.Tensor(2*self.n_iter, 2*(len(self._dists)+1), self.d_model, rank))
            self.W = nn.Parameter(torch.Tensor(2*self.n_iter, 2*(len(self._dists)+1), self.n_head, rank))
            nn.init.normal_(self.U, std = 1/math.sqrt(rank))
            nn.init.normal_(self.V, std = 1/math.sqrt(rank))
            nn.init.normal_(self.W, std = 1/math.sqrt(rank))
        else:
            self.ternary = nn.Parameter(torch.Tensor(2*self.n_iter, 2*(len(self._dists)+1), self.d_model, self.d_model, self.n_head))
            nn.init.normal_(self.ternary)
    ...
    def forward(self, x, mask):
        ...
        for iteration in range(self.n_iter):
            ...
            if self.async_update:
                ...
                # Calculate 2nd message for different dists
                second_order_message_F = oe.contract('zia,zjb,kabc,kij->zcij',*[q_z, q_z, ternary[iteration*2], distmask], backend='torch')
                ...
                # Calculate 2nd message for different dists
                second_order_message_G = oe.contract('zjb,zcij,kabc,kij->zia', *[q_z, q_h, ternary[iteration*2+1], distmask], backend='torch')
                if not self.block_msg:
                    second_order_message_G = second_order_message_G + oe.contract('zjb,zcji,kbac,kji->zia', *[q_z, q_h, ternary[iteration*2+1], distmask], backend='torch')
...
class NoUnaryHeadProbEncoder(nn.Module):
    """
    Head Probabilistic Transformer encoder. Z nodes have no unary potential other than initialization.
    """
    def __init__(self, 
            d_model: int = 32, 
            n_head: int = 10, 
    ...
    def forward(self, x, mask):
        ...
        for iteration in range(self.n_iter):
            ...
            if self.async_update:
                ...
                # Update
                q_z = cache_qz * self.damping_Z + (second_order_message_G) * (1-self.damping_Z) / self.regularize_Z
                q_z = self.dropout_z(q_z)
