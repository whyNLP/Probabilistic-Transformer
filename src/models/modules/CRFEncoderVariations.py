...
class DynamicEtaHeadProbEncoder(nn.Module):
    """
    Head Probabilistic Transformer encoder.
    Recover ternary score only when calculating ternary norm.

    eta = t / (t+2), follow the Frank-Wolfe setting.
    """
    def __init__(self, 
            d_model: int = 32, 
            n_head: int = 10, 
    ...
    def forward(self, x, mask):
        ...
        for iteration in range(self.n_iter):
            ...
            stepsize_Z = self.stepsize_Z * (2 / (iteration + 2))
            stepsize_H = self.stepsize_H * (2 / (iteration + 2))
            ...
            if self.async_update:
                ...
                # Normalize
                q_z = ( (1-stepsize_Z) * cache_norm_qz + stepsize_Z * self.norm_func(q_z) ) if iteration else self.norm_func(q_z)
                ...
                # Normalize
                q_h = ( (1-stepsize_H) * cache_norm_qh + stepsize_H * self.norm_func(q_h) ) if iteration else self.norm_func(q_h)
                ...
                # Normalize
                q_z, q_h = (1-stepsize_Z) * cache_norm_qz + stepsize_Z * self.norm_func(q_z), \
                           (1-stepsize_H) * cache_norm_qh + stepsize_H * self.norm_func(q_h)
