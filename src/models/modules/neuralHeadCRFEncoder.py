...
class NeuralHeadProbEncoder(nn.Module):
    """
    Head Probabilistic Transformer encoder with neural parameterization.
    """
    def __init__(self, 
            d_model: int = 32, 
            n_head: int = 10, 
        ...
        :param use_td: control tensor decomposition. Options:
                         ...
                         - 'k:{k1}:{k2}:uv:{rank}': each 'head' decompose to 2 matrices W = U @ V.
                           where U = relu(E @ W_U1) @ W_U2, E.shape = (d_model, k1), 
                           W_U1.shape = (k1, k2), W_U2.shape = (k2, rank). Use two
                           numbers to set k, one number to set the rank, e.g. 'k:128:128:uv:64';
                         - 'hk:{k1}:{k2}:uv:{rank}': same as the above, but no embedding sharing
                           between channels.
        ...
        elif self.use_td.startswith('k:'):
            k1 = int(self.use_td.split(':')[1])
            k2 = int(self.use_td.split(':')[2])
            rank = int(self.use_td.split(':')[-1])
            self.E = nn.Parameter(torch.Tensor(2*(len(self._dists)+1), self.d_model, k1))
            self.linearU1 = nn.Parameter(torch.Tensor(k1, k2, self.n_head))
            self.linearU2 = nn.Parameter(torch.Tensor(k2, rank, self.n_head))
            self.linearV1 = nn.Parameter(torch.Tensor(k1, k2, self.n_head))
            self.linearV2 = nn.Parameter(torch.Tensor(k2, rank, self.n_head))
            nn.init.normal_(self.E, std = 1/math.sqrt(k1))
            nn.init.uniform_(self.linearU1, a = -1/math.sqrt(k1), b = 1/math.sqrt(k1))
            nn.init.uniform_(self.linearU2, a = -1/math.sqrt(k2), b = 1/math.sqrt(k2))
            nn.init.uniform_(self.linearV1, a = -1/math.sqrt(k1), b = 1/math.sqrt(k1))
            nn.init.uniform_(self.linearV2, a = -1/math.sqrt(k2), b = 1/math.sqrt(k2))
        elif self.use_td.startswith('hk:'):
            k1 = int(self.use_td.split(':')[1])
            k2 = int(self.use_td.split(':')[2])
            rank = int(self.use_td.split(':')[-1])
            self.E = nn.Parameter(torch.Tensor(2*(len(self._dists)+1), self.d_model, k1, self.n_head))
            self.linearU1 = nn.Parameter(torch.Tensor(k1, k2, self.n_head))
            self.linearU2 = nn.Parameter(torch.Tensor(k2, rank, self.n_head))
            self.linearV1 = nn.Parameter(torch.Tensor(k1, k2, self.n_head))
            self.linearV2 = nn.Parameter(torch.Tensor(k2, rank, self.n_head))
    ...
    def forward(self, x, mask):
        ...
        if self.use_td.startswith('uv:'):
            ...
            x = oe.contract('kae,erh->karh', *[self.E, self.linearU1], backend='torch')
            x = F.relu(x)
            U = oe.contract('karh,rsh->kash', *[x, self.linearU2], backend='torch')
            y = oe.contract('kae,erh->karh', *[self.E, self.linearV1], backend='torch')
            y = F.relu(y)
            V = oe.contract('karh,rsh->kash', *[y, self.linearV2], backend='torch')
            ternary = oe.contract('kadc,kbdc->kabc', *[U, V], backend='torch')
        elif self.use_td.startswith('hk:'):
            x = oe.contract('kaeh,erh->karh', *[self.E, self.linearU1], backend='torch')
            x = F.relu(x)
            U = oe.contract('karh,rsh->kash', *[x, self.linearU2], backend='torch')
            y = oe.contract('kaeh,erh->karh', *[self.E, self.linearV1], backend='torch')
            y = F.relu(y)
            V = oe.contract('karh,rsh->kash', *[y, self.linearV2], backend='torch')
            ternary = oe.contract('kadc,kbdc->kabc', *[U, V], backend='torch')
        elif self.use_td.startswith('uvw:'):
