...
class ProbEncoder(nn.Module):
    ...
    """
    def __init__(self, d_model=32, d_label=32, n_iter=4, zero_edge=False, damping=0, norm='softmax'):
        """
        ...
        self.ternary = nn.Parameter(torch.ones(2, self.d_model, self.d_model, self.d_label))
    ...
    def forward(self, x, mask):
        ...
        for _ in range(self.n_iter):
            ...
            second_order_message = torch.einsum('zjb,zijc,abc->zia', [q_z, q_y, self.ternary[0]]) + \
                                   torch.einsum('zjb,zjic,abc->zia', [q_z, q_y, self.ternary[1]])
            
            if self.damping:
                q_y = cache_q_y * self.damping + \
                      (torch.einsum('zia,zjb,abc->zijc',[q_z, q_z, self.ternary[0]]) + \
                      torch.einsum('zja,zib,abc->zijc',[q_z, q_z, self.ternary[1]])) * (1-self.damping)
                q_z = cache_q_z * self.damping + (unary + second_order_message) * (1-self.damping)
            else:
                q_y = torch.einsum('zia,zjb,abc->zijc',[q_z, q_z, self.ternary[0]]) + \
                      torch.einsum('zja,zib,abc->zijc',[q_z, q_z, self.ternary[1]])
...
class ProbEncoderTD(nn.Module):
    ...
    """
    def __init__(self, d_model=256, d_label=64, rank=256, n_iter=4, zero_edge=False, damping=0, norm='softmax'):
        """
...
class ProbEncoderWithDistance(nn.Module):
    """
    Basic Probabilistic Transformer encoder.
    """
    def __init__(self, 
...
class ProbEncoderTDWithDistance(nn.Module):
    """
    Probabilistic Transformer encoder with tensor decomposition.
    """
    def __init__(self, 
