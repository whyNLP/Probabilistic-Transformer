...
class MultiZHeadProbEncoder(nn.Module):
    """
    ...
    def __init__(self, 
        ...
        """
        assert d_model % n_head == 0, f"d_model ({d_model}) have to be a multiple of n_head ({n_head})"

        super().__init__()
        self.d_model = d_model
        self.n_head = n_head
        self.d_z = d_model // n_head
        self.n_iter = n_iter
        ...
        if self.use_td.startswith('uv:'):
            ...
            self.U = nn.Parameter(torch.Tensor(2*(len(self._dists)+1), self.d_z, rank, self.n_head))
            self.V = nn.Parameter(torch.Tensor(2*(len(self._dists)+1), self.d_z, rank, self.n_head))
            nn.init.normal_(self.U, std = 1/math.sqrt(rank))
            nn.init.normal_(self.V, std = 1/math.sqrt(rank))
        elif self.use_td.startswith('uvw:'):
            rank = int(self.use_td.split(':')[-1])
            self.U = nn.Parameter(torch.Tensor(2*(len(self._dists)+1), self.d_z, rank))
            self.V = nn.Parameter(torch.Tensor(2*(len(self._dists)+1), self.d_z, rank))
            self.W = nn.Parameter(torch.Tensor(2*(len(self._dists)+1), self.n_head, rank))
            nn.init.normal_(self.U, std = 1/math.sqrt(rank))
            nn.init.normal_(self.V, std = 1/math.sqrt(rank))
            nn.init.normal_(self.W, std = 1/math.sqrt(rank))
        else:
            self.ternary = nn.Parameter(torch.Tensor(2*(len(self._dists)+1), self.d_z, self.d_z, self.n_head))
    ...
    def forward(self, x, mask):
        ...
        mask1d = ~mask1d.view(batch_size, max_len, 1, 1)
        mask2d = ~mask2d.view(batch_size, max_len, max_len)
        ...
        ## Unary score
        unary = x # (batch_size, max_len, n_head, d_z)
        unary = x.reshape(batch_size, max_len, self.n_head, self.d_z)
        ...
        q_z = unary.clone()
        q_h = torch.ones(batch_size, max_len, max_len).to(x.device)
        ...
        for iteration in range(self.n_iter):
            ...
            if self.async_update:
                ...
                second_order_message_F = oe.contract('zica,zjcb,kabc,kij->zij',*[q_z, q_z, ternary, distmask], backend='torch')
                ...
                # Calculate 2nd message for different dists
                second_order_message_G = oe.contract('zjcb,zij,kabc,kij->zica', *[q_z, q_h, ternary, distmask], backend='torch')
                if not self.block_msg:
                    second_order_message_G = second_order_message_G + oe.contract('zjcb,zji,kbac,kji->zica', *[q_z, q_h, ternary, distmask], backend='torch')
        ...
        q_z = q_z.reshape(batch_size, max_len, self.d_model)
        return q_z
