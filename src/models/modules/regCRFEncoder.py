...
class RegularizedProbEncoder(nn.Module):
    """
    Basic Probabilistic Transformer encoder.
    With Regularized Frank-Wolfe, an extend version.
    """
    def __init__(self, 
            d_model: int = 32, 
            d_label: int = 32, 
        ...
        :param regularize_Z: regularization for updating Z.
                'regularize_Z' and 'regularize_Y' are regularizations for MFVI. See 
    ...
    def forward(self, x, mask):
        ...
        ## Recover ternary score
        if self.use_td.startswith('uv:'):
            expr_G1 = oe.contract_expression('zia,zjb,kadc,kbdc,kij->zijc',*[unary.shape, unary.shape, self.U[0], self.V[0], distmask], constants=[2,3,4], optimize='optimal')
            expr_G2 = oe.contract_expression('zia,zjb,kbdc,kadc,kji->zijc',*[unary.shape, unary.shape, self.U[1], self.V[1], distmask], constants=[2,3,4], optimize='optimal')
  
            expr_F1 = oe.contract_expression('zjb,zijc,kadc,kbdc,kij->zia', *[unary.shape, (batch_size, max_len, max_len, self.d_label), self.U[0], self.V[0], distmask], constants=[2,3,4], optimize='optimal')
            expr_F2 = oe.contract_expression('zjb,zjic,kadc,kbdc,kij->zia', *[unary.shape, (batch_size, max_len, max_len, self.d_label), self.U[1], self.V[1], distmask], constants=[2,3,4], optimize='optimal')
            expr_F3 = oe.contract_expression('zjb,zjic,kbdc,kadc,kji->zia', *[unary.shape, (batch_size, max_len, max_len, self.d_label), self.U[0], self.V[0], distmask], constants=[2,3,4], optimize='optimal')
            expr_F4 = oe.contract_expression('zjb,zijc,kbdc,kadc,kji->zia', *[unary.shape, (batch_size, max_len, max_len, self.d_label), self.U[1], self.V[1], distmask], constants=[2,3,4], optimize='optimal')
        elif self.use_td.startswith('uvw:'):
            ternary = oe.contract('tkad,tkbd,tkcd->tkabc', *[self.U, self.V, self.W], backend='torch')
        else:
            ternary = self.ternary
        ...
        for _ in range(self.n_iter):
            ...
            if self.async_update:
                ...
                second_order_message_G = expr_G1(q_z, q_z) + expr_G2(q_z, q_z)
                
                # Update
                q_y = cache_qy * self.damping_Y + second_order_message_G * (1-self.damping_Y) / self.regularize_Y # * self.d_model
                ...
                second_order_message_F = expr_F1(q_z, q_y) + expr_F2(q_z, q_y) + expr_F3(q_z, q_y) + expr_F4(q_z, q_y)

                # Update
                q_z = cache_qz * self.damping_Z + (unary * 0 + second_order_message_F) * (1-self.damping_Z) / self.regularize_Z
                ...
                # Update
                q_y = cache_qy * self.damping_Y + second_order_message_G * (1-self.damping_Y) / self.regularize_Y # * self.d_model
                q_z = cache_qz * self.damping_Z + (unary + second_order_message_F) * (1-self.damping_Z) / self.regularize_Z
                q_y = self.dropout_y(q_y)
                q_z = self.dropout_z(q_z)
